...
flowchart TB
    ...
    subgraph AWS["AWS Cloud — us-east-1"]
        ...
        subgraph MON["Detection Layer"]
            CW["📊 CloudWatch\nAlarms\n• CPU > 80%\n• StatusCheckFailed\n• Disk > 85%"]
            EB["⚡ EventBridge\nRule: route alarm\nstate changes"]
            SNS["📨 SNS Topic\nlab-itsm-alerts"]
        end

        subgraph NATIVE["Track B — AWS-Native ITSM"]
            IM["🚨 SSM Incident Manager\n─────────────────\nResponse Plans\nEngagements\nTimeline + Post-Incident"]
            SC["📦 AWS Service Catalog\n─────────────────\nPortfolio: IT Services\nProduct: Dev EC2 Request\n(CloudFormation-backed)"]
            CM["📋 SSM Change Manager\n─────────────────\nChange Templates\nApproval workflow\nAutomated runbooks"]
            CFG["🗂️ AWS Config\n─────────────────\nResource inventory\n= CMDB equivalent"]
        end
    ...
    subgraph SN["ServiceNow PDI  (SaaS)"]
        INC["🎫 Incident\nservice-now.com\nINC0001234"]
        CAT["📦 Service Catalog\nNew Laptop Request\nAWS Dev Instance"]
        CHG["📋 Change Request\nCHG0001234\nApproval workflow"]
        CMDB["🗂️ CMDB\nCI records synced\nfrom AWS"]
    ...
    USER -- "Reports issue" --> INC
    USER -- "Self-service request" --> CAT
    ENG -- "Works ticket" --> INC
    ENG -- "Submits change" --> CHG
    ...
    SNS -- "Track A" --> LAMBDA
    LAMBDA -- "REST API\nauto-create incident" --> INC

    SNS -- "Track B" --> IM

    CHG -. "Approved change\ntriggers" .-> CM
    CM -- "Executes runbook" --> EC2
    CAT -. "Provisions via" .-> SC
    SC -- "CloudFormation" --> EC2

    CFG -. "Resource inventory\nsync" .-> CMDB
    IAMROLE -. "Grants access" .-> LAMBDA
    IAMROLE -. "Grants access" .-> CM
